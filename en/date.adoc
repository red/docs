...
Many different input formats are supported, including all the Rebol date formats and most of ISO date standard.
----
<date>/<time>
<date>/<time><zone>
<date>T<time>
...
* `year` and `day` fields are interchangeable, but for low year values, the year can be used in first position only if its value >= 100 and less than the value of the third field. When that rule is not satisfied, the third field is considered the year. Negative years should always be specified in last position.
...
* `/precise`: get time with higher precision (1/60th of a second on Windows, micro-seconds on Unix)
