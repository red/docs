...
NOTE: the corners rounding effect takes precedence over eventual `line-join` setting.
...
----
shadow <offset>
shadow <offset> <color> <inset>
shadow <offset> <blur> <inset>
shadow <offset> <blur> <color> <inset>
shadow <offset> <blur> <spread> <inset>
shadow <offset> <blur> <spread> <color> <inset>
shadow off

<offset> : offset of the shadow (pair!).
<blur>   : (optional) the blur radius (integer!).
<spread> : (optional) the spread radius (integer!).
<color>  : (optional) starting angle in degrees (tuple! word!).
<inset>  : (optional) `inset` (word!)
----

*Description*

Draws one or more shadow effect(s) on all subsequent drawn shapes (applies to all drawing commands). Issuing several `shadow` commands will draw multiple shadows. If the command's argument is the `off` keyword, all shadow effects are turned off on subsequent drawn shapes. The optional `inset` argument will change the shadow from an outer shadow (outset) to an inner shadow.
...
NOTE: the rounding effect in `box` command takes precedence over eventual `line-join` setting.
