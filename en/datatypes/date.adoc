...
Using `to`, the first position can be used for the `year` *only* if its value is >= 100 and its value is greater than the value of the third field. When that rule is not satisfied, the third field is considered the `year`. 
...
[NOTE]
Setting a `/zone` value to a date that lacks a `time` value will result in `/zone` defaulting to 0:0:0. 
