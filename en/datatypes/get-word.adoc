...
A `get-word!` value gets an unevaluated word value from the context to which it is bonund.

`Gett-word!` is a member of the following typsets: `any-word!`, `all-word!`
...
A colon `:` followed by one or more Unicode characters, or ASCII characters from the set:  `! & ' * + - . < = > ? _ ` | ~` 
