...
|selected| integer!, pair!, object!, date! | no| some| For lists types, index of currently selected element. For text inputs, selected text. For windows, focused face. For calendar, selected date.
...
|Facet| Description
|`type`| `'calendar`
|`selected`| `date!` value that represents selected day.
|===

[cols="1, 1, 3", options="header"]
|===
|Event type| Handler| Description
|`select`| `on-select`| Occurs when a date in the calendar is selected. `selected` facet refers to *old* selected date.
|`change`| `on-change`| Occurs after a `select` event. `selected` facet refers to the *new* selected date.
|===

[NOTE]
====
* By default, `selected` facet is initialized to "today" date.
