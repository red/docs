...
Blocks are the aggregate structures for values of all possible types. They are used to represent data or code that can be evaluated at any point in runtime.
...
`Block!` is part of the `series!` and `any-block!` typesets.
