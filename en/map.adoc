...
<key>   : hashed key, accepted types are: any-word!, any-string!, integer!, float!, char!
...
* values are not_reduced_, so construction syntax is required for some special values, like logic!.
...
* Like hash! and block!, map! is [underline]#case-preserving#, and [underline]#case-insensitive for lookup# by default.
