...
`float!` spans a range from `-1.7E+308` to `+1.7E+308`.

`float!` returns #INF on overflow by design.
...
All comparators can be applied on floats: `=, ==, <>, >, <, >=, &lt;=, same?`. In addition, `min`, and `max` are also supported.
