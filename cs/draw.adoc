...
Kosočtvercový gradient se vykreslí ve směru od ohniska k okraji kosočtverce, definovaného horním a dolním rohem. Počáteční barva se vykreslí v ohnisku, koncová barva na okraji čtyřúhelníka.
...
Definuje, zda nové matice v následných operacích jsou pre-multiplied (`prepend`) nebo post-multiplied (`append`, default mode) ke stávající matici.
...
Při použití volby 'rotate pen' či 'rotate fill-pen' je rotace provedena vzhledem ke stávající volbě 'pen' či 'fill-pen'.
...
Při použití volby 'scale pen' či 'scale fill-pen' je zvětšení provedeno vzhledem ke stávající volbě 'pen' či 'fill-pen'.
...
Při použití volby 'translate pen' či 'translate fill-pen' je posunutí provedeno vzhledem ke stávající volbě 'pen' či 'fill-pen'.
...
Při použití volby 'skew pen' či 'skew fill-pen' je zkosení provedeno vzhledem ke stávající volbě 'pen' či 'fill-pen'.
...
NOTE: Všechny kreslící příkazy používají implicitně absolutní souřadnice. Použití 'lit-word' verze příkazu přepne na relativní souřadnice.
...
<angle>    : úhel mezi počátečním a koncovým bodem oblouku ve stupních (integer! float!).
...
Kreslí kubickou Bezierovu křivku, danou třemi body. Počátečním bodem je aktuální pozice pera.
...
Fill-pen také podporuje překonané API, které existuje jenom kvůli kompatibilitě s Rebol/Draw; v nových skriptech Redu by se nemělo používat.
...
NOTE: gradient lze zadat až 256 ti barvami.
