...
Jsou-li pole time a/nebo zone vysazena (unset), jsou vynechána. U negativních datumových hodnot se pro lepší čitelnost používá separátor `/` místo `-`.
...
Na rozdíl od literálové skladby se tímto způsobem (runtime creation) vytváří datumová hodnota při běhu programu.
