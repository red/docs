...
| `integer!`
| Konverze na tento datový typ může ústit v přetečení datového typu, pokud celočíselná část má více než 10 číslic. Desetinná část je odvržena.
...
Při porovnávání hodnot typu `money!` laxními operátory (`=`, `<>`) nebo striktním operátorem (`==`) či operátorem identity (`=?`), musí jít o stejnou měnu nebo jeden z operandů musí reprezentovat generickou měnu; v tomto případě jsou porovnávány pouze částky.
...
* seznam měn je typu `append-only` a může být rozšířen o měnu definovanou uživatelem.
...
* příkaz `system/options/money-digits: n` určí počet desetinných míst pro výstup `form` nebo `mold`. Možná hodnota `n` je v rozsahu `[0, 5]`
...
>> print [sign? mony negative? mony positive? mony zero? mony]
-1 true false false

>> print [even? mony odd? mony]
false true
...
== true
----
Vytvoření hodnoty typu `money!` ze zadané měny a peněžní částky:
----
>> as-money TBD
...
----
