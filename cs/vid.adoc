...
VID automaticky vytvoří kontejnerový piškot pro popis zadaných piškotů. Implicitně je kontejnerový piškot typu `window`.
...
NOTE: Klíčové slovo `react` keyword může být použito při nastavení kontejneru i při výběru možností pro `face` - viz podrobný popis v odstavci <<anchor:_react, zde>>.
...
VID umisťuje piškoty do kontejneru podle jednoduchých pravidel:
...
Piškot lze vložit do aktuální pozice dispozice pouhým uvedením jména existujícího typu piškotu nebo dostupného stylu.
...
Nastaví aspekt `draw` piškotu na seznam příkazů dialektu Draw. Viz dokumentaci [Draw dialect](draw.html).
...
Nastaví aspekt `font` piškotu na nový objekt typu `font!`. Objekt font! je popsán link:view.html#_font_object[zde].
...
Nastaví aspekt `para` novému objektu `para!`. Objekt para! je popsán link:view.html#_para_object[zde].
...
----
Platný seznam názvů událostí lze nalézt link:view.html#_actors[zde].
