...
* make typeset! [link:datatypes/unset.adoc[unset!]]
...
// Typ prvků v řadě je závislý na datovém typu řady, např. řada typu `block!` může obsahovat dle definice bloku (link:datatypes/block.adoc[block!]) hodnoty libovolného typu. Řada typu `string` - opět podle definice řetězce (link:datatypes/string.adoc[string!]) může obsahovat pouze hodnoty typu `char`.
