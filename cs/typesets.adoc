...
Typ prvků v řadě (series) je závislý na jejím datovém typu, např. řada typu `block` může obsahovat dle definice (link:datatypes/block.adoc[block!]) hodnoty libovolného typu. Řada typu `string`(link:datatypes/string.adoc[string!]) může obsahovat pouze hodnoty typu `char`.
