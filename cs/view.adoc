...
Systém Red/View (nebo prostě View či View engine) je grafický systém jazyka Red. Jeho cíle lze shrnout do těchto bodů:
...
Piškot (tvář neboli face) je objekt, který vznikne buď kompilací funkce `layout` z dialogu VID (viz Dialekt VID) nebo se vytvoří klonováním objektu face! doplněném alespoň jedním platným názvem `typu`:
----
button: make face! [type: 'button]
----

Kromě předdefinovaných typů piškotů (area, box, base, button, camera, drop-down, drop-list, field, group-box, h1-h5, check, panel, progress, radio, slider, screen, text, text-list, tab-panel, window; viz odstavec 7) lze vytvořit piškot vlastní.

Grafický objekt na obrazovce se vytvoří zobrazením (funkce `view`) virtuálního stromu. Virtuální strom je sestavován při kompilaci z piškotů (faces). Každý piškot (face object) je propojen se zobrazeným grafickým komponentem dvoucestnou vazbou.
...
Piškoty jsou organizovány do stromu, který odpovídá hierarchii jejich vztahů v zobrazení. Vztahy u stromu jsou odvozeny z:
...
_Tento typ piškotu může být použit pro zavedení libovolné grafické komponenty uživatele._
...
|*text*	| string!		| no	|  all	| Text, zobrazený v piškotu. Atributy textu jsou určeny aspekty `font` a `para`.
...
* *probublávání (bubbling) události*: událost postupuje od čelního piškotu k rodičovskému piškotu window. Pro každý piškot je volán lokální ovladač události.
...
Instaluje funkci globálního ovladače, která umí předzpracovat události před tím, než se dostanou k ovladačům piškotu. Pro každou událost jsou volány všechny globální ovladače, takže kód těla ovladače potřebuje optimalizovat rychlost a využití paměti. Je-li jako argument poskytnut blok, je konvertován na funkci s použitím konstruktoru `function`.
