...
=== Aspekty anchor:options-facet[]

Aspekt `options` obsahuje volitelné aspekty, používané pro specifické chování:
...
== Strom piškotů anchor:face-tree[]
...
image:../images/face-tree.png[]



== Typy piškotů anchor:face-types[]
...
== Životní cyklus piškotu anchor:face-life-cycle[]
...
== Dvojsměrné propojení anchor:two-way-binding[]
...
== Události anchor:events[]

=== Názvy událostí anchor:event-names[]
...
===Datový typ Event! anchor:event!-datatype[]
...
* Všechna pole (kromě `type`) jsou pouze pro čtení. Nastavení aspektu `type` provádí pouze interně prostředí View.
...
=== Tok událostí anchor:event-flow[]
...
=== Globální ovladače událostí anchor:global-event-handlers[]
...
== Objekt system/view anchor:system-view-object[] 
...
== Připojení View anchor:including-view-component[]
...
Poznámka: Při použití konzol, automaticky generovaných binárkou `red`, je komponenta View implicitně připojena na podporujících platformách; pole Needs v záhlaví tedy není požadováno ve skriptech, spouštěných z těchto konzol.

== Extra funkce anchor:extra-functions[]
