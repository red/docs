...
Piškot (tvář neboli face, také  widget) je objekt, který vznikne buď kompilací funkce `layout` z dialogu link:vid.adoc[Red/VID] nebo se vytvoří klonováním objektu face! doplněném alespoň jedním platným názvem `typu`:
----
button: make face! [type: 'button]
----

Kromě předdefinovaných typů piškotů (area, base, button, camera, drop-down, drop-list, field, group-box, h1-h5, check, panel, progress, radio, slider, screen, text, text-list, tab-panel, window - viz odstavec 7) lze vytvořit piškot vlastní.

Grafický objekt na obrazovce se vytvoří zobrazením (funkce `view`) virtuálního stromu. Virtuální strom je sestavován při kompilaci z piškotů (faces). Každý piškot je propojen se zobrazeným grafickým komponentem dvoucestnou vazbou.
