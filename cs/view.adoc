...
Piškoty - tváře neboli faces (od slova interface) jsou klony výchozího objektu `face!`. 
Vlastnosti, stavy i aktivity piškotů jsou popsány sadou parametrů (polí), zvaných 
_facet_, neboli `aspekt`, jichž je aktuálně 23 - viz link:view.html#_typy_piškotů[dole]. 

Nový piškot (face) se vytvoří klonováním objektu `face!` a zadáním  *alespoň* platného názvu `typu`.
