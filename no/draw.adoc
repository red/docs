...
<color> : ny farge (tuple! word!).
----

*Beskrivelse*

Velger fargen for tegneoperasjoner. Alle former vil da tegnes i den valgte fargen til pennen er satt til `off`.
...
<color1/N> : liste over farger for gradienten (tuple! word!).
<offset>   : (optional) forskyvning av gradientfargen (float!).
<start>    : (valgfritt) startpunkt (pair!). 
<end>      : (valgfritt om ikke <start>) sluttpunkt (pair!).
<spread>   : (valgfritt) spredningsområde (word!).
----

*Beskrivelse*

Setter en lineær gradient som brukes til tegneoperasjonene. Følgende verdier aksepteres for spredningsmetoden: `pad`, `repeat`, `reflect` (for øyeblikket er `pad` the samme som `repeat` på Windows-plattformen).

Hvis start og sluttpunkt er angitt, definerer de en linje som gradienten tegnes langs. Hvis ikke, tegnes gradienten langs en horisontal linje inne i formen som tegnes.
